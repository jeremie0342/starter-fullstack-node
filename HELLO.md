# Bonjour Skilluv

Welcome to your first Skilluv project!

You are here because your trade works on the web - *web, blockchain* - or audits and tests one: *security* and *quality* learn on this same surface, which exposes the classic attack points (JWT, upload, session).

Edit this file by adding a line under `## My introduction` with:
- Your first name or handle
- Your city (optional)
- What you want to learn

Then: commit + push + open a Pull Request from `main` to `showcase`.

Skilluv will automatically detect your PR and unlock your first badge.

---

## Bonjour Skilluv (français)

Bienvenue dans ton premier projet Skilluv !

Tu es ici parce que ton métier travaille le web - *web, blockchain* - ou l'audite et le teste : *security* et *quality* apprennent sur cette même surface, qui expose les points d'attaque classiques (JWT, upload, session).

Modifie ce fichier en ajoutant une ligne sous `## Ma présentation` avec :
- Ton prénom ou pseudo
- Ta ville (facultatif)
- Ce que tu veux apprendre

Ensuite : commit + push + ouvre une Pull Request de `main` vers `showcase`.

Skilluv détectera automatiquement ta PR et débloquera ton premier badge.

---

## My introduction / Ma présentation

<!-- Add your line here / Ajoute ta ligne ici ↓ -->

Jeremie. Je construis Skilluv : un compagnonnage tech où ce que les gens savent faire se prouve par des artefacts réels plutôt que par un diplôme. Backend Rust. Ce que je veux apprendre : le faire tenir en production.
